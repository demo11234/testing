...
export class UserController {
   ...
   */
  @ApiTags('User Module')
  @ApiOperation({ summary : "Login User with given Wallet Address or Create new Account with given Wallet Address" })
  ...
  async createUser(@Body() createUserDto: CreateUserDto, @Response() response): Promise<any> {
    try {
      ...
      if(user) {
        const details = await this.authService.createUserToken(createUserDto.walletAddress, user);
        return this.responseModel.response(details, ResponseStatusCode.OK, true, response);
      }
      else {
        user = await this.userService.createUser(createUserDto);
        const details = await this.authService.createUserToken(createUserDto.walletAddress, user);
        return this.responseModel.response(details, ResponseStatusCode.CREATED, true, response);
      }
    } catch (error) {
  ...
  @ApiResponse({ status: ResponseStatusCode.CONFLICT, description: ResponseMessage.USER_EXISTS_WITH_GIVEN_EMAIL_ADDRESS })
  @ApiResponse({ status: ResponseStatusCode.CONFLICT, description: ResponseMessage.USER_EXISTS_WITH_GIVEN_USER_NAME })
  ...
  async updateUser(@Body() updateUserDto: UpdateUserDto, @Response() response, @Request() request): Promise<any> {
    try {
      ...
      if(email) {
        ...
        if(user) {
          return this.responseModel.response(ResponseMessage.USER_EXISTS_WITH_GIVEN_EMAIL_ADDRESS, ResponseStatusCode.CONFLICT, false, response);
        }
      }
      ...
      if(userName) {
        ...
        if(user) {
          return this.responseModel.response(ResponseMessage.USER_EXISTS_WITH_GIVEN_USER_NAME, ResponseStatusCode.CONFLICT, false, response);
        }
      }

      const user = await this.userService.update(request.user.walletAddress, updateUserDto);
      if(!user) {
        return this.responseModel.response(ResponseMessage.USER_DOES_NOT_EXISTS_WITH_GIVEN_WALLET_ADDRESS, ResponseStatusCode.NOT_FOUND, false, response);
      }
      else {
  ...
  async getUserDetailsByUserName(@Body() userNameDto : UserNameDto, @Response() response): Promise<any> {
    try {
      ...
      const user = await this.userService.findUserByUserName(userName);
      if(!user) {
        return this.responseModel.response(ResponseMessage.USER_DOES_NOT_EXISTS_WITH_GIVEN_USERNAME, ResponseStatusCode.NOT_FOUND, false, response);
      }
      else {
        return this.responseModel.response(user, ResponseStatusCode.OK, true, response);
      }
